...
// SlideDeckFormat selects the deck layout NotebookLM generates.
...
func (c *Client) CreateSlideDeckWithOptions(ctx context.Context, projectID, instructions string, sourceIDs []string, format SlideDeckFormat) (string, error) {
	...
	args := intmethod.EncodeCreateSlideDeckArgs(projectID, sourceIDs, instructions, "en", intmethod.SlideDeckFormat(format))
	call := rpc.Call{
		ID:         "R7cb6c",
		NotebookID: projectID,
		Args:       args,
