...
| `nlm analytics <notebook-id>` | Show notebook analytics time series |
...
| Command | Description |
| --- | --- |
| `nlm label list <notebook-id>` | List labels (autolabel clusters) in a notebook |
| `nlm label generate <notebook-id>` | Recompute autolabel clusters for a notebook |
| `nlm label create <notebook-id> <name> [emoji]` | Create a new manual label on a notebook |
...
| `nlm label unlabeled <notebook-id>` | Apply existing labels to currently-unlabeled sources |
| `nlm label relabel-all <notebook-id>` | Re-cluster everything (UI's "Relabel all") |
...
| Command | Description |
| --- | --- |
| `nlm audio list <notebook-id>` | List audio overviews for a notebook |
...
| Command | Description |
| --- | --- |
| `nlm artifact list <notebook-id>` | List artifacts in notebook |
...
| `nlm audio-suggestions <notebook-id>` | Suggest audio-overview blueprints (emit JSON lines; pipe to create-audio) |
...
| Command | Description |
| --- | --- |
| `nlm chat list [notebook-id]` | List chat sessions (server-side when a notebook is given) |
...
| `nlm chat delete <notebook-id>` | Delete server-side chat history |
...
| `nlm account [set <key> <value>]` | Show or update the authenticated user's NotebookLM account (ZwVcOc / hT54vc) |
