...
`extractChatPayload` (`notebooklm/client.go:4938`) has two branches:
...
Add a table test in `notebooklm` that feeds a **proto-decodable**
...
No network method on `notebooklm.Client` takes a `context.Context`. Every HTTP call goes
...
`notebooklm.Client` is configured by functional options **and** mutable `Set*` setters
...
- **Completed:** the high-level client now lives at module-root
  `package notebooklm`. Callers import `github.com/tmc/nlm/notebooklm`; the
  package path and package name now describe the service rather than a generic
  "API."
...
## 6. `notebooklm` — public high-level client (finding #6)

**Status: package promotion and documentation complete.**

The high-level client is the public library surface. It has a package comment,
name-led documentation for exported declarations, examples, and an
external-module compile test. Keep wire-protocol qualifications in method
documentation: this package is source-level Go API, but the upstream NotebookLM
service remains private and can change independently.
...
| **Citation resolution** | `resolve_citations.go`, `source_match.go` (~530) | `notebooklm` | `internal/richrender` or `internal/citations` |
| **Source ingestion helpers** | `add_sources.go`, `source_read.go`, `source_flags.go` bodies (~1,300) | `notebooklm` | fold into `notebooklm` or a `sources` subpackage |

**Evidence this is clean to do:** the render files import only `notebooklm` and `pb` — no
...
functions on local types. The only coupling is to `notebooklm`, and that points the right way
— a renderer *should* depend on the client's types, not vice versa. There is no import
cycle risk as long as `notebooklm` does not depend back on the render package (it does not, and
must not).
...
1. **Create `internal/richrender`.** Move the rendering cluster there. Export the entry
   ...
   the `notebooklm.RichContent` constructor can be dropped entirely — the render package ends
   up proto-native.
...
`notebooklm/client.go` (6935 lines) is the *other* monolith. Split it
