...
| Audio/video results | **request pass**: GetAudioFormats sentinel and R7cb6c audio/video/slide envelopes match captured corpus shapes; result variants still need typed comparison | **video projection pass (fixture)**; audio/slide result adapters remain unverified | **CreateVideoOverview now projects generated Artifact; default CreateSlideDeck returns the same artifact ID projection**; audio behavior remains legacy | pending | **pass (corpus/fixture)**: generated GetAudioFormats and CreateUniversalArtifact video/slide encoders pin captured shapes | **GetAudioFormats, CreateVideoOverview, and default no-instructions CreateSlideDeck switched**; audio and custom-instruction slide paths remain mixed | retained: audio instruction placement, slide custom-instruction field, and polymorphic result parsers need corpus-backed adapters |
