...
| Account state | **fixture pass**: generated `Account` decodes the compact ZwVcOc status payload and quota fields | **pass (fixture)**: `parseAccountStatusProto` matches the legacy `AccountStatus` projection | **pass (unit)**: public quota/tier values are unchanged; compact error handling remains covered by legacy tests | pending | generated `GetOrCreateAccount` encoder remains available; status request stays raw empty args pending request capture | **GetAccountStatus response switched to generated Account decode**; request and `GetOrCreateAccount` remain mixed | retained: `parseAccountStatus` remains the legacy oracle until complete corpus and CLI gates pass |
| Audio/video results | **request pass**: GetAudioFormats sentinel and R7cb6c video envelope match captured corpus shapes; result variants still need typed comparison | **video projection pass (fixture)**; audio result adapter remains unverified | **CreateVideoOverview now projects generated Artifact to the existing public result**; audio behavior remains legacy | pending | **pass (fixture)**: generated GetAudioFormats and CreateUniversalArtifact video encoders pin captured shapes | **GetAudioFormats and CreateVideoOverview switched**; audio create/get/delete paths remain mixed | retained: audio instruction placement and polymorphic result parsers need corpus-backed adapters |
