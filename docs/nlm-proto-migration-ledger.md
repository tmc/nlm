...
| Artifacts | **request pass**: gArtLc ListArtifacts request matches the corpus; response variants remain typed/legacy mixed | **partial (corpus)**: generated and legacy projections agree on 40 observed artifacts except four rendered slide artifacts whose URL-derived READY state is not represented by the generated enum | legacy response parser retained for URL/state semantics; generated request path is behavior-preserving at the transport boundary | **compile pass**: generated preview types are now consumed by `getArtifact`; output fixture equivalence still required | **pass (fixture)**: generated ListArtifacts encoder pins the full corpus request | **ListArtifacts request switched**; response parsing and other artifact RPCs remain mixed | retained: artifact polymorphism and rendered-URL state promotion (4 corpus mismatches) still require a tested adapter before switching |
